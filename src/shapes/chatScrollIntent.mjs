...
//
// Intent comes only from a live input gesture plus its scrollTop delta. A
// virtualizer re-anchor can move scrollTop with a stable content height, so
// geometry alone cannot distinguish the reader from the renderer.
 ...
 * @param {{scrolledUp:boolean, hardLocked:boolean, geometryReconciliation:boolean, userInputActive?:boolean}} state
...
export function decideFollowTransition(sample, state) {
  ...
  // up-scroll: a live input gesture moved top past the jitter eps while content
  // height stayed fixed. A resize can shift scrollTop in either direction.
  ...
  const movedUp = state.userInputActive === true && top < lastTop - UP_JITTER_EPS && heightStable && gap > TRUE_BOTTOM_EPS
  // down-to-bottom: a live input gesture moved top past the jitter eps to within
  // EPS of the true bottom. The shrink case this once guarded — a window shrink collapsing the
  ...
  const movedDownToBottom = state.userInputActive === true && top > lastTop + UP_JITTER_EPS && gap <= FOLLOW_BOTTOM_EPS
