/**
 * IsolatedCanvasClipPanel — shared copy-store TLDraw panel that shows a clipped
...
interface IsolatedCanvasClipPanelProps {
  mainEditor: Editor
  ...
  recordsOverride?: TLRecord[]
  children?: React.ReactNode
}

export function IsolatedCanvasClipPanel({
  mainEditor,
  bounds,
  shapeUtils,
  tools,
  ...
  const hasRecordsOverride = !!recordsOverride
  ...
  const store = useMemo(() => {
    const allRecords = recordsOverride ?? mainEditor.store.allRecords()
    const docRecords = allRecords.filter(r => {
      if (!isKnownShape(r)) return false
      return recordsOverride ? isDocRecord(r) : shouldSyncToCopy(r)
    })
      .map(r => {
  ...
  useEffect(() => {
    ...
    const docRecords = recordsOverride.filter(r => isDocRecord(r) && isKnownShape(r))
      .map(r => readOnly && r.typeName === 'shape'
        ? ({ ...r, isLocked: true } as TLRecord)
        : r)
    const toRemove = store.allRecords()
      .filter(r => r.typeName === 'shape' || r.typeName === 'asset')
      .map(r => r.id)
    store.mergeRemoteChanges(() => {
      if (toRemove.length) store.remove(toRemove)
      if (docRecords.length) store.put(docRecords)
    })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [recordsOverride, readOnly, store])
  ...
  useEffect(() => {
    ...
    return unsub
  }, [editor, mainEditor.store, store, lockCamera, readOnly, hasRecordsOverride])
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [bounds?.x, bounds?.y, bounds?.w, bounds?.h, lockCamera, store, mainEditor, hasRecordsOverride])
  ...
  }, [mainEditor, mainEditor.store, store, lockCamera, readOnly, customGestureActiveRef, hasRecordsOverride])
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Best effort HUD reset; the next editor tick will retry.
      log.warn('canvas-clip', 'failed to force HUD browse tool', { error: String(error) })
    }
  }, [editor, lockCamera])
    ...
    editor.registerExternalContentHandler('url', async (info) => {
      ...
      if (url?.startsWith('data:image/')) {
        try {
          ...
          // Best effort drag conversion; failing drops should not crash the HUD.
          log.warn('canvas-clip', 'failed to convert HUD data image drop', { error: String(error) })
        }
        return
      }
    ...
    mainEditor.registerExternalContentHandler('url', async (info) => {
      ...
      if (url?.startsWith('data:image/')) {
        try {
          ...
          // Best effort drag conversion; falling back to no-op avoids bookmark crashes.
          log.warn('canvas-clip', 'failed to convert main data image drop', { error: String(error) })
        }
        return
      }
      if (!url?.startsWith('http://') && !url?.startsWith('https://')) return
