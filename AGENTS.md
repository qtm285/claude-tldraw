...
- `pgrep -fl fleet-daemon` to see what's running, and `ps eww <pid> | tr ' ' '\n' | grep
  TLDA_ENV` to see which environment a daemon serves.
...
**A defensive branch is not evidence that the state it defends against can occur.** Check the constraint before believing the guard. `_pickLiveHolder` ranked agents sharing a friendly name by which was awake, and its own comment explained why that mattered for waking the right one — so two agents and a chief read it as load-bearing on wake routing and escalated it as a product decision. A partial unique index (`friendly_name WHERE dead = 0`) had made the collision unrepresentable since the day it was added. Instrumented on the old build, the branch was entered four times and **never once with more than one row**; the state it ranked could not be constructed, because the database refuses it. It was 38 lines of dead code reaching into main-thread state, and it blocked a much larger change.

The mirror of this is just as expensive: **a method that resembles the raw SQL beside it will differ from it.** Four for four in `fleet-store.mjs` — one merges where its callers replace, one adds seat projection, one hydrates `0/1` into booleans, one coalesces where the raw sites clear. The resemblance is the trap, and it gets *more* convincing the closer you look. What appears to be a mechanical sweep of duplicated SQL is a pile of individual semantic judgments.
...
settings live in `daemon.yaml`; bots live in `bots.yaml`; ordinary CLI
