...
class InputsManager {
  ...
  updateFromEvent(info) {
    ...
    const viewport = info.viewportId && info.viewportId !== import_TLViewport.DEFAULT_VIEWPORT_ID ? (0, import_state.unsafe__withoutCapture)(() => this.editor.getViewport(info.viewportId)) : null;
    const screenBounds = viewport?.screenBounds ?? this.editor.store.unsafeGetWithoutCapture(import_tlschema.TLINSTANCE_ID).screenBounds;
    const {
      x: cx,
      y: cy,
      z: cz
    } = viewport?.camera ?? (0, import_state.unsafe__withoutCapture)(() => this.editor.getCamera());
