...
\end{equation}
which measures how much $\frac{1}{n}\sum_i m(X_i)$ can vary over $m \in \model$, net of a quadratic penalty on the treated sample. The maximal imbalance of the minimax balancing weights has a clean characterization in terms of $\omega$. At the optimal bias-variance tradeoff parameterized by $s > 0$,
\begin{equation}
...
\end{equation}
where $V^{\textup{eff}} = N_1^{-2}\sum_{i=1}^n (W_i + \overset{\neg}{W}_i \theta(X_i)^2) v_{W_i}(X_i)$ is the optimal variance and $r(x) = \theta(x) - \floor{\theta(x)}$ is the fractional part of the propensity odds $\theta(x) = e(x)/\{1-e(x)\}$.
...
\end{equation}
where $\norm{f}_{L_2(\Pn)}^2 = n^{-1}\sum_{i=1}^n f(X_i)^2$. Because the functional $\frac{1}{n}\sum_i m(X_i)$ does not depend on the values of $m$ at control units, the optimizer places zero weight there, and the constraint effectively acts on the treated sample only. The two formulations are related by the Fenchel--Legendre transform: $\omega(s) = \sup_{\delta \ge 0}\{\bar\omega(\delta) - \delta^2/(2s)\}$, or equivalently $\bar\omega(\delta) = \inf_{s > 0}\{\omega(s) + \delta^2/(2s)\}$, with the optimal $s$ and $\delta$ linked by $\delta = s\,\bar\omega'(\delta)$.
