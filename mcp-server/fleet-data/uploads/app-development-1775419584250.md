...
Reference for agents working on the tlda viewer or fleet dashboard UI. Read before starting any UI/app task.
...
Worktrees must be inside the project directory (`.worktrees/` subdirectory). Not `/tmp`. Relative paths in `vite.config.ts` (e.g. `../fleet/dashboard`) break when the worktree is in `/tmp` because macOS symlinks `/tmp → /private/tmp`, causing path resolution to fail. See `agent-guide.md` for the full workflow.
...
| Fleet dashboard | 5199 | Fleet server (SSE, REST API, SQLite) |
...
**Start fleet:** `cd ~/work/fleet && node dashboard/server.mjs`
