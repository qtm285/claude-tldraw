...
\caption{Modulus of continuity and its connection to approximability, for a Mat\'ern-3/2 model (blue) and a Gaussian RBF model (orange) on the same sample ($n = 500$, $d = 1$, sine propensity). \emph{Left:} $\omega(\delta)$ (solid) and its linearization $\omega'(\delta)\,\delta$ (dashed). The shaded gap is the bias $\omega(\delta) - \omega'(\delta)\,\delta$. The Gaussian model has a more linear modulus and smaller bias at each~$\delta$, but its RKHS is more restrictive. \emph{Right:} For each model, $\delta_{\max}(B)$---the largest~$\delta$ at which the bound~\eqref{eq:omega-bound} stays below~$n^{-1/2}$---as a function of the competitor model norm~$B$. The Mat\'ern-3/2 model, for which the propensity score is easy to approximate, plateaus quickly. The Gaussian model requires much larger~$B$ but eventually reaches further right on the left panel.}
\label{fig:modulus}
\end{figure}
...
\omega(\delta) \le C\,r^2 + \varepsilon\,\delta,
\end{equation}
where $r$ is the fixed-point radius satisfying~\eqref{eq:least-squares-fixed-point}. The first term is the local complexity of the model---the same localization as in Section~\ref{sec:role-of-augmentation}. The second is the centering error: $\gammaipw$ is not in the model, so the centered empirical process is not exactly mean-zero, and this costs~$\varepsilon\,\delta$.

The competitor norm~$B$ enters through the approximation error $\varepsilon(B) = \inf\{\norm{g - \gammaipw}_{L_2} : g \in \model,\, \norm{g}_\model \le B\}$. Larger~$B$ allows better approximation---smaller~$\varepsilon$---which shrinks the second term and permits the bound to hold at larger~$\delta$. Define $\delta_{\max}(B)$ as the largest~$\delta$ at which~\eqref{eq:omega-bound} stays below~$n^{-1/2}$. Figure~\ref{fig:modulus} (right) plots $\delta_{\max}(B)$ for each model: given a competitor's model norm, it locates the reader on the left panel. The Mat\'ern-3/2 model approximates the propensity score easily, so $\delta_{\max}$ plateaus at moderate~$B$. The Gaussian model, whose RKHS is more restrictive, requires much larger~$B$ to reach comparable~$\delta$. For bias negligibility---$\imbalance_\model(\hgamma) = o_p(n^{-1/2})$---it suffices that $C\,r^2 + \varepsilon(B)\,\delta^* = o(n^{-1/2})$. For nonparametric models, $\varepsilon(B) \to 0$ as $B \to \infty$, so this holds for any fixed operating point. The conclusion is stronger than in Section~\ref{sec:role-of-augmentation}: there, augmentation with a consistent estimator was needed to convert an $O_p(n^{-1/2})$ bound on maximal imbalance into $o_p(n^{-1/2})$. Here, the minimax balancing weights achieve it without augmentation.
...
and let $r > 0$ satisfy the fixed-point inequality~\eqref{eq:least-squares-fixed-point}. If there exists a function $g \in \model$ with $\norm{g - \gammaipw}_{L_2(P)} \le \varepsilon$, then $\imbalance_\model(\hgamma) \le C\, r^2 + \varepsilon\,\delta^*$ where $\delta^*$ is the estimator's operating point. If $\model$ is a nonparametric (Definition~\ref{def:nonparametric}) Donsker class of uniformly bounded functions, then $\varepsilon \to 0$ as $\norm{g}_\model \to \infty$, so $\imbalance_\model(\hgamma) = o_p(n^{-1/2})$.\footnote{For RKHS models, the approximation rate $\varepsilon(B)$ can be made explicit in terms of eigenvalue decay rates; see Appendix~\ref{app:modulus-equivalence}.}
\end{proposition}
