...
PlaybackFrame implements `onDragShapesIn` in the shape util — when a shape is dragged onto the frame on the canvas, it gets reparented as a child. The +HUD button is a convenience shortcut that calls `editor.createShape` with `parentId: frameId`, but the same result can be achieved by dragging any fleet-chat or fleet-agents shape from the canvas into the frame's bounds.

The existing children (seen throughout this report) were created by clicking +HUD, which creates:
- fleet-chat at `x=0, y=CHROME_H, w=60%frame, h=frame_h-CHROME_H`
- fleet-agents at `x=chatW+4, y=CHROME_H, w=40%frame-4, h=frame_h-CHROME_H`

A manually dragged fleet-chat would similarly become a child and inherit the frame's playback context — its `useFleetEvents` hook detects the `frameId` and switches to playback mode.

### +HUD children visible in both panels

![HUD at t=30 full frame](http://localhost:5199/api/files/r18-frame-t30-with-hud-1775274075988.jpeg)

The two child shapes created by +HUD, showing at t=30:00 in the Building Playback session. Left panel: fleet-chat with the substantive PhD/HCI/journalistic project conversation visible in full. Right panel: fleet-agents listing all 5 agents from the recording. Both are clipped to the frame's bounds (bug fix #3 from `e72f0ab`).
