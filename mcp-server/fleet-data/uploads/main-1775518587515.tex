...
\end{equation}
When $\hat \mu_1$ is cross-fit, the second term on the right is $o_p(n^{-1/2})$ if $\hat \mu_1$ is consistent. The first---the inner product---treats the two nuisance errors symmetrically. And the conditional bias is negligible if and only if this inner product is.
