...
Live terminal card connected to the `chip-fixes` agent's tmux session. Orange header with agent name and attention reason, dark terminal body showing real output, all integration test results below.

![Terminal attention card — live xterm.js connected to chip-fixes agent, all 9 tests passing](terminal-cards-screenshot.png)
