...
\end{equation}
which measures the worst-case imbalance in the model, penalized by the empirical variance of $m$ on the treated sample. In our setting, the linear estimators are weighted estimators $\hat\psi_1 = n^{-1}\sum_i W_i \gamma(X_i) Y_i$, and the worst-case bias over the model is our maximal imbalance $\imbalance_\model(\gamma)$. Minimax balancing weights minimize a penalized version of this worst-case bias, making them instances of Donoho's minimax linear estimator \citep{donoho1994statistical, armstrong2018optimal}.
