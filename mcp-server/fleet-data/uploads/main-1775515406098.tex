...
The approaches above---the plug-in comparison in Section~\ref{sec:plugin} and the augmentation argument in Section~\ref{sec:role-of-augmentation}---both analyze the bias by comparing it to the behavior of the true inverse propensity weights $\gammaipw$. An alternative decomposition targets the bias more directly. We can expand the conditional bias term in Equation~\ref{eq:weights_error_aug} into two terms,
\begin{equation}
...
This sharper analysis is what makes \emph{optimal double robustness} possible: efficient inference whenever \emph{either} $\gammaipw$ or $\mu_1$ is smooth enough to be estimable at faster than $n^{-1/4}$ rate---a requirement that cannot be improved \citep{mukherjee2017semiparametric, robins2009semiparametric}. \citet{newey2018cross} show that AIPW with sieve nuisance estimators achieves this guarantee, and \citet{kennedy2020optimal} extends this to general linear nuisance estimators. The same principle applies to balancing weights \citep{hirshberg2019minimax}.
\end{remark}
