...
## NEW TASK: Silent Infrastructure Death

### What happened (April 5, ~4:35 PM):
Fleet server went down. Fleet MCP disconnected — all agents lost chat capability silently. tlda server also went down. Skip had to manually discover each failure, restart each service, and run `/mcp` to reconnect fleet tools. No notification, no auto-recovery, no visibility into what was broken.

### What Skip said:
> "I can't be fucking restarting all my agents MCPs and all this shit over and over"
> "Our shit cannot fail silently. I lose work when that happens."
> "I need the ability to look into this myself easily and fix it and agents need to fucking see it all instead of seeing one piece at a time."

### What needs to exist:

**1. Health visibility** — A single place (on the tlda canvas or a CLI command) that shows the status of ALL services:
- tlda server (port 5176) — up/down
- Whisper server (port 8178) — up/down  
- Fleet server (port 5199) — up/down
- Fleet MCP connection — connected/disconnected per agent
- Yjs sync — connected/disconnected per doc
- Build status — last build time, any errors

**2. No silent death** — When a service goes down:
- Agents should see it immediately (not discover it when a tool call fails)
- The viewer should show it (not silently serve stale content or fail to load)
- Something should attempt auto-restart or at minimum scream loudly

**3. Self-service recovery** — Skip needs to be able to:
- See what's broken at a glance
- Fix it with one command (not 5 separate restarts)
- Not lose work in the process (no reloads without asking)

### What exists today:
- `tlda server start` — starts tlda + whisper, but doesn't monitor them
- `tlda server status` — point-in-time check, not continuous
- Fleet server has a PID lock file, that's it
- No auto-restart on crash
- No health dashboard
- No cross-service status visibility
- When fleet MCP disconnects, agents just silently stop receiving messages

### Implementation approach (for whoever picks this up):
- A watchdog process that monitors all services and auto-restarts on crash
- A health status shape or panel in the tlda viewer showing green/red for each service
- Fleet MCP should reconnect automatically on server restart (or at minimum notify the agent that it needs `/mcp`)
- `tlda doctor` CLI command that checks everything and fixes what it can
