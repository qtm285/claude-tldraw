...
Bounding the inner product by the product of $L_2$ norms $\norm{\delta\gamma}_{L_2(\Pn)}\norm{\dm}_{L_2(\Pn)}$ gives \emph{rate double robustness}: the conditional bias is negligible when the product of the two estimation rates is $o_p(n^{-1/2})$, regardless of which factor is small. This bound is crude---when both nuisances converge at the same rate, that rate must exceed $n^{-1/4}$---but the symmetry it reflects is real. Efficient inference is possible whenever \emph{either} $\gammaipw$ or $\mu_1$ is estimable at a rate faster than $n^{-1/4}$, a requirement that cannot be improved \citep{mukherjee2017semiparametric, robins2009semiparametric}. This \emph{optimal double robustness} is achieved when first-order conditions from the estimation procedure---least squares, regularized regression, or the lasso---make the errors $\dgamma$ and $\dm$ approximately orthogonal \citep{vermeulen2015bias,tan2010bounded,hirshberg2019minimax,bradic2019sparsity}. \citet{newey2018cross} show this for AIPW with sieve nuisance estimators, \citet{kennedy2020optimal} extends it to general linear nuisance estimators, and the same principle applies to balancing weights \citep{hirshberg2019minimax}.
