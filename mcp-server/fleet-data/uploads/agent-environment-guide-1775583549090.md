...
Skip's screen is typically split into three regions:

### Left: Fleet Chat
The fleet chat panel shows a scrolling timeline of agent messages — task updates, status reports, bash command logs. This is where your `chat()` messages appear. Skip reads these, responds via voice transcription, and uses this to coordinate work across agents. It renders markdown, so format your messages with headers, bold, lists, and code blocks.

### Center: Agent Terminals
Multiple agent terminals run side by side. Skip can see your terminal output (bash commands, Python scripts, build logs) but **cannot interact with it** — he can't type into your terminal. If you need input from Skip, use `chat()`. Terminal output is for your work, not for communication.

### Right: The Paper (tlda Viewer)
This is the core of the environment. Your LaTeX output renders here as **SVG pages on a TLDraw canvas** — not a PDF viewer, not Preview.app. The rendered paper shows:
- Full mathematical typesetting (theorems, corollaries, remarks, display math, inline math)
- Clickable cross-references (equation numbers, proposition refs)
- Proper figure and table layout
- Page-by-page vertical stacking

When Skip reviews on iPad, this is what he sees — the same rendered pages, but with pen/annotation tools overlaid. He draws circles around problems, highlights text, drops sticky notes with KaTeX math.
