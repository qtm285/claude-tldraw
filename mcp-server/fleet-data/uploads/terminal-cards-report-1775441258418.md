...
## Skip's feedback (not yet implemented)

**Terminal cards should persist in chat history.** They don't vanish after dismiss. They become static — showing what happened (the attention event, what action was taken). They're part of the chat record, like any other message. Dismiss = close the live terminal, but the card stays as a static record of the event.

**Dismiss = freeze, not remove.** Click X closes the live WebSocket and kills the xterm instance, but the card stays in chat showing the last terminal output as frozen static text. Header (agent, reason, timestamp) stays. The card is now a static record in chat history.
