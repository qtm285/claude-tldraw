...
1. ✅ **Toggle on** — overlay appears with dashed border, label, handles
2. ✅ **Proportional resize** — dragging handle scales all fleet shapes by same factor
3. ✅ **Toggle off (Done)** — overlay removed, shapes stay at new positions
...
**No "Done" button.** Exit layout mode by clicking outside the transient container. Click off = done. Remove the "Done" button entirely.

**Container should look like a normal tldraw shape.** No special dashed purple border, no "Layout Mode" label. If it's an actual tldraw shape, it should look like one. The current implementation looks like a modal overlay, not a shape.
