...
sufficiently rich model $\model$. \begin{definition}[Nonparametric model]
\label{def:nonparametric}
A model $\model$ is \emph{nonparametric} if it can be inflated to approximate any square integrable function:
for every $m$ with $\norm{m}_{L_2(P)} < \infty$ and $\epsilon>0$, there exists $\tilde m$ with $\norm{\tilde m}_\model \le \alpha$ satisfying $\norm{m-\tilde m}_{L_2(P)} < \epsilon$.
\end{definition}
...
\imbalance_\model(\hgamma) = \omega(\delta_\sigma) - \delta_\sigma\,\omega'(\delta_\sigma),
\end{equation}
the $y$-intercept of the tangent line to $\omega$ at the estimator's operating point $\delta_\sigma$ (Figure~\ref{fig:modulus}, left). When $\gammaipw$ is in the model, $\omega = 0$ and the bias vanishes. When it is not, bounding the bias reduces to bounding~$\omega$.
\FloatBarrier
\begin{figure}[h!]
\centering
\makebox[0.48\textwidth]{\includegraphics[width=0.48\textwidth]{figure/modulus-tangent-line.pdf}}%
\hfill
\makebox[0.48\textwidth]{\includegraphics[width=0.48\textwidth]{figure/bias-bound.pdf}}
\caption{\emph{Left:} Modulus of continuity $\omega(\delta)$ (solid) and its linearization $\omega'(\delta)\,\delta$ (dashed) for two models on the same sample ($n = 500$, $d = 1$). The shaded gap is the bias $\omega(\delta) - \omega'(\delta)\,\delta$. The smoother model (Gaussian RBF, green) has a more linear modulus and smaller bias than the rougher model (Mat\'ern-3/2, blue). \emph{Right:} Bias bound~\eqref{eq:omega-bound} as a function of the competitor model norm~$B$ (Mat\'ern-3/2, $n = 500$, $\sigma = 3$). The bound equals the local complexity $C\,r^2$ (blue) plus $(\sigma^2/n)\,B$, crossing $n^{-1/2}$ (dotted) at $B \approx 2.4$ (dashed). The actual bias (dark) is well below. A good bound exists when $B$ is small enough---i.e., when $\gammaipw$ is well-approximable in the model.}
\label{fig:modulus}
\end{figure}
...
\end{equation}
where $r(\delta)$ is the fixed-point radius satisfying~\eqref{eq:least-squares-fixed-point} at scale~$\delta$. The first term is the local complexity of the model---the same localization as in Section~\ref{sec:role-of-augmentation}, illustrated in Figure~\ref{fig:width-geometry}. The second is the approximation cost: the model norm of the competitor~$g$ times $\sigma^2/n$. The condition $\varepsilon \le r(\delta)$ asks only that the approximation be close enough to fall within the localization radius---when it does, the centering error is absorbed into the local complexity. Figure~\ref{fig:modulus} (right) illustrates the bound for several values of~$B$.

To match the local complexity rate~$C\,r^2$, Proposition~\ref{prop:bias-balance} requires $B \le C\,r^2 n/\sigma^2$ and $\varepsilon \le C\,r^2\sqrt{n}/\sigma$: the competitor must approximate $\gammaipw$ at a model norm and $L_2$ error scaled to the localization. When both conditions hold, the total is $C\,r^2 = o_p(n^{-1/2})$. But this is a strong standard. For negligibility alone---$o_p(n^{-1/2})$---it suffices that $(\sigma^2/n)\,B = o(n^{-1/2})$, i.e., $B = o(\sqrt{n}/\sigma^2)$. For nonparametric models with constant $\sigma$, any finite~$B$ suffices: the model can approximate any square-integrable~$\gammaipw$, and the approximation condition is automatic. The conclusion is stronger than in Section~\ref{sec:role-of-augmentation}: there, augmentation with a consistent estimator was needed to convert an $O_p(n^{-1/2})$ bound on maximal imbalance into $o_p(n^{-1/2})$. Here, the minimax balancing weights achieve it without augmentation.
...
\caption{\textit{Left}: In-sample versus out-of-sample maximal imbalance as a function of the tuning parameter $\sigma^2$, for $n \in \{100, 200, 400\}$ (bold: $n = 200$). Balancing weights are fit on sample~A and evaluated on both sample~A (in-sample, blue) and an independent sample~B (out-of-sample, red). Gray lines mark IPW imbalance. Shaded regions indicate three regimes: undersmoothing, bias--variance tradeoff, and constant weights. The gap between the curves is the in-sample advantage exploited by Proposition~\ref{prop:bias-balance}. \textit{Right}: Location of the bias--variance tradeoff regime on the $\sigma^2$ axis for three propensity score shapes across a range of sample sizes. Each stick spans the $\sigma^2$ values in this regime for one propensity at one sample size; the undersmoothing regime lies below each stick and the constant-weights regime lies above. Bold sticks use a Mat\'ern-3/2 model; faint sticks use a Gaussian RBF model on the same data. The smoother model has a wider tradeoff regime. Faint curves in the background show the three propensity score functions.}
\label{fig:insample-outsample}
\end{figure}

But this is an \emph{in-sample} phenomenon. If we fit weights on one sample and evaluate balance on another, we lose the cancellation that makes aggressive tuning work. Figure~\ref{fig:insample-outsample} illustrates this: in-sample imbalance decreases monotonically as $\sigma$ shrinks, while out-of-sample imbalance---the imbalance that controls bias when weights are cross-fit---first decreases then increases as the weights overfit the training sample. The gap between the two curves is precisely the in-sample advantage of balancing weights---the advantage that cross-fitting, which evaluates weights on held-out data, cannot access.

The upward shift of the tradeoff regime with $n$ in Figure~\ref{fig:insample-outsample} reflects the distinction between in-sample and out-of-sample balance. In-sample balance improves as $\sigma$ shrinks, because smaller $\sigma$ gives the weights more freedom to match the sample. Out-of-sample balance, by contrast, requires the weights to estimate the inverse propensity score function, and the rate-optimal regularization for this estimation grows with $n$: taking $\sigma \propto \sqrt{n}\, r$ for the fixed-point radius $r$~\eqref{eq:least-squares-fixed-point} gives the minimax rate of convergence for the weight function \citep[Section~3.3]{hirshberg2021augmented}. Because $r \to 0$ slower than $n^{-1/2}$, $\sigma^2 \propto n r^2$ grows with $n$. This means that tuning for in-sample balance is forgiving---any sufficiently small $\sigma$ works---while tuning for out-of-sample balance targets a moving window whose location depends on both the sample size and the model.

\begin{remark}[Augmentation and convex models]
\label{remark:augmentation-convex}
Where does this leave augmentation? For convex models, the minimax linear theory tells us that augmentation is unnecessary for near-optimality, and the results above show this extends to asymptotic efficiency with no restrictions on undersmoothing. The largest benefits of augmentation appear in nonconvex models, such as sparse models, where linear estimators are no longer near-optimal \citep{cai2004minimax}. In simulations using sparse models, augmentation can cut root mean squared error by half or more \citep[Section~6, Setup~2]{hirshberg2021augmented}.
\end{remark}

\subsection{The inner product perspective}%
\label{sec:inner-product}

We have been using an asymmetric framing of the bias, bounding the worst case over the outcome model. The conventional framing of this estimation problem is more symmetric. The connection is the decomposition of the conditional bias in Equation~\ref{eq:weights_error_aug} into an inner product of the weight error $\dgamma = \hgamma - \gammaipw$ and the outcome error $\dm$, plus a remainder:
\begin{equation}
    \label{eq:aug_bias_dr}
    \frac{1}{n}\sum_{i=1}^n W_i \hgamma(X_i) \dm(X_i) - \frac{1}{n}\sum_{i=1}^n \dm(X_i) = \frac{1}{n}\sum_{i=1}^nW_i \delta\gamma(X_i) \dm(X_i) \;+\; \frac{1}{n}\sum_{i=1}^n \left\{W_i \gammaipw(X_i) - 1\right\}\dm(X_i).
\end{equation}
When $\hat \mu_1$ is cross-fit, the second term on the right is $o_p(n^{-1/2})$ if $\hat \mu_1$ is consistent. The first---the inner product---treats the two nuisance errors symmetrically. And the conditional bias is negligible if and only if this inner product is. An inner product can only be small if the factors are small or nearly orthogonal, so bounding maximal imbalance over the model---as in the preceding sections---amounts to ensuring that $\dgamma$ is either small or nearly orthogonal to every function in the model that is not itself small. The preceding sections established this in an asymmetric framing; the inner product formulation makes the underlying symmetry explicit.

A common way to use this decomposition is to bound the inner product by the product of norms $\norm{\delta\gamma}_{L_2(\Pn)}\norm{\dm}_{L_2(\Pn)}$, giving \emph{rate double robustness}: the bias is negligible when this product is $o_p(n^{-1/2})$, regardless of which nuisance is estimated more precisely. This captures a real phenomenon---the two nuisances can compensate for each other---but it also suggests a false limitation. When both converge at the same rate, that rate must exceed $n^{-1/4}$, and this is often taken as a fundamental barrier. It is not.

When weights or outcomes are estimated by projection---least squares onto a basis, regularized regression, or the lasso---first-order conditions show that the errors $\dgamma$ and $\dm$ are approximately orthogonal to the fitted subspace. Orthogonality makes the inner product small even when neither norm is---something the product-of-norms bound cannot see. This \emph{improved double robustness} \citep{vermeulen2015bias,tan2010bounded} sharpens the rate conditions beyond what Cauchy--Schwarz gives. Taken to its logical conclusion, this leads to \emph{optimal double robustness}. In smooth nonparametric models, \citet{mukherjee2017semiparametric} and \citet{robins2009semiparametric} show that efficient inference is possible whenever \emph{either} $\gammaipw$ or $\mu_1$ is smooth enough to be estimable at a rate faster than $n^{-1/4}$, and that this requirement cannot be improved. They achieve it with estimators based on higher-order influence functions; subsequent work showed that much simpler estimators have the same property. \citet{newey2018cross} show this for AIPW with sieve nuisance estimators, and \citet{kennedy2020optimal} extends it to general linear nuisance estimators. In high-dimensional models, where the story is somewhat different, \citet{bradic2019sparsity} establishes an analogous optimality result.

\begin{remark}[Regression for orthogonality]
\label{remark:regression-for-orthogonality}
The symmetry of the inner product means the maximal imbalance logic can be reversed. \citet{wang2020debiased} observe that rather than choosing weights orthogonal to outcome model errors, we can choose the outcome regression so that its error is orthogonal to weight errors. This yields $\sqrt{n}$-consistent estimation under assumptions on the propensity score rather than the outcome model. See Appendix~\ref{app:regression-orthogonality} for a sketch in our notation.
\end{remark}
...
\end{aligned}
\end{equation}
This is the same phenomenon that makes AIPW preferable to IPW with the true weights $\gammaipw$: mean-zero imbalance costs nothing in bias but inflates variance. Here, it is the \emph{change} in $\mu$-imbalance from rounding, rather than the $\mu$-imbalance itself. Because the excess variance $\Delta V$ is the mean-squared change in $\mu$-imbalance, controlling the worst-case change in imbalance uniformly over a model $\model \ni \mu$ controls this cost. To avoid it, we need to round with negligible change in imbalance. Given a model $\model$ for $\mu$, we can ask for negligible change in imbalance by solving
the following integer program, which minimizes the rounding-induced change uniformly over $m\in\model$.
\begin{equation}
\label{eq:round}
...
Rounding is thus essentially free in terms of bias. The remaining question is what it costs in terms of variance---that is, how much larger the variance of a matching estimator must be, relative to the efficient variance, simply because its weights are constrained to be integers. The cost of matching can be characterized precisely. For any regular matching estimator,
\begin{equation}
...
Combining the rounding bound of Proposition~\ref{prop:rounding} with the asymptotic expansion from Proposition~\ref{proposition:clt}, we get a matching analogue of our earlier CLT: a matched difference in means based on balance-preserving rounding is asymptotically normal, with a simple variance estimator.
...
\end{equation}
This is how existing matching-for-balance methods work. \citet{kallus2016generalized} proposed this integer-constrained formulation under the name \emph{generalized optimal matching}, and \citet{zubizarreta2014matching}'s \emph{cardinality matching} takes a similar approach. Our rounding results provide new inferential guarantees for both. Specifically, Proposition~\ref{prop:rounding} implies that the integer program~\eqref{eq:integer-sbw} is feasible whenever its relaxation achieves uniformly negligible imbalance---as it does for nonparametric BUEI classes of bounded functions, because these are Donsker and Proposition~\ref{prop:bias-balance} applies---and Corollary~\ref{cor:rounding} then delivers asymptotic normality and a simple variance estimator.

\begin{corollary}
\label{cor:integer-sbw}
If $\model$ is a nonparametric (Definition~\ref{def:nonparametric}) class of uniformly bounded functions with bounded uniform entropy integral, then for an appropriately chosen sequence of budgets $B_n \ll n^{-1/2}$, the integer program \eqref{eq:integer-sbw} has a solution with probability tending to one and the corresponding difference in means estimator \smash{$\hat\psi^{match}=\frac{1}{N_1}\sum_{i=1}^n W_iY_i - \overset{\neg}W_i \round{\htheta}_i Y_i$} satisfies the asymptotic expansions \eqref{eq:asymptotic-linearity-rounded} and \eqref{eq:asymptotic-normality-rounded}, with variance achieving equality in the matching cost bound~\eqref{eq:matching-cost}.
\end{corollary}
...
However, if our sole concern is efficiency while strictly limiting the number of responses we need to record, integer weights may not be the best form of sparsity.
We could, for example, replace the integrality constraint in~\eqref{eq:integer-sbw} with a constraint on the number of nonzero weights, producing sparse but not necessarily integer-valued solutions. These sparse weighting ideas are an interesting area for future work.
...
The penalized modulus~\eqref{eq:donoho-liu-bias} used in Section~\ref{sec:doing-better} is related by Lagrangian duality to the constrained formulation of \citet{donoho1994statistical} and \citet{armstrong2018optimal}. The constrained modulus is
\begin{equation}
...
Because $\model$ has bounded uniform entropy integral, it is Donsker, so Proposition~\ref{prop:bias-balance} applies: the non-integer version of \eqref{eq:integer-sbw} achieves $o_p(n^{-1/2})$ imbalance.
...
For nonparametric BUEI classes of bounded functions, the conditions of Proposition~\ref{prop:matching-achievable}$'$ are satisfied by the minimax balancing weights.

\begin{corollaryprime}{\ref{cor:integer-sbw}}
Let $\model$ be a nonparametric (Definition~\ref{def:nonparametric}) class of uniformly bounded functions with bounded uniform entropy integral. Assume $\E[1/e(X)] < \infty$ and $\hat\theta v_0, (1{-}2\hat r)v_0 \in L^2(P)$.
