...
Math notes are a major communication format — they render as chips in chat, sticky notes on canvas, collapsed dots. The state matters:

- **Uncollapsed note near camera** = Skip is actively reading this
- **Collapsed dot** = seen it, moved on
- **Recently expanded** = just opened, probably in response to something

Include note collapsed/expanded state in the context metadata so agents know what Skip is engaging with.
