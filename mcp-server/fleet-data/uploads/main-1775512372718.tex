...
\end{equation}
When $\omega$ is approximately linear---$\omega(s) \approx c\,s$ for some constant $c$---the maximal imbalance is approximately zero. For a finite-dimensional model with $k$ basis functions and $k \le n$, exact balance is achievable and $\omega$ is exactly linear. For a nonparametric model, exact balance is not achievable, but $\omega$ is approximately linear when the model's effective dimension at the relevant scale is small relative to $n$.

What determines the effective dimension is the same quantity that determines the difficulty of nonparametric regression: the fixed-point radius $r$ satisfying~\eqref{eq:least-squares-fixed-point}. In Section~\ref{sec:role-of-augmentation}, this radius controlled how well we could estimate the regression function $\mu_1$. \citet{kong2025asymptotics} and \citet{hirshberg2026bregman} show that it also controls how much the modulus can deviate from linearity: the deviation is at most $C\,r^2$. The reason is the same in both cases---both balance and regression error are governed by how much functions in the model can correlate with noise at the sample points. But the conclusion is stronger: Section~\ref{sec:role-of-augmentation} needed augmentation with a consistent estimator to convert $O_p(n^{-1/2})$ imbalance into $o_p(n^{-1/2})$. Here, the minimax balancing weights achieve $o_p(n^{-1/2})$ imbalance on their own. This is the same quantity illustrated in Figure~\ref{fig:width-geometry}, where the crossing point determines $r$.
...
For RKHS models, the approximation condition in Proposition~\ref{prop:bias-balance} can be made explicit in terms of eigenvalue decay rates; see Appendix~\ref{app:modulus-equivalence}.
