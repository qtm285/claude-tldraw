...
The results above establish that balancing weights can beat the true weights, but they do not tell us by how much. There is a long tradition of work on minimax estimation of linear functionals \citep{ibragimov1985nonparametric, donoho1991geometrizing, donoho1994statistical} that studies precisely this kind of problem: estimating a linear functional of an unknown function known to lie in a convex class. Its central object is the \emph{modulus of continuity} $\omega$, in terms of which the maximal imbalance of the minimax balancing weights satisfies\footnote{This penalized formulation of $\omega$ is equivalent by Lagrangian duality to the constrained formulation used by \citet{donoho1994statistical} and \citet{armstrong2018optimal}. See Appendix~\ref{app:modulus-equivalence}.}
\FloatBarrier
\begin{figure}[h!]
\centering
\includegraphics[width=0.45\textwidth]{figure/modulus-tangent-line.pdf}
\caption{Modulus of continuity $\omega(s)$ (solid) and its linearization $\omega'(s)\,s$ (dashed) for two models on the same sample ($n = 500$, $d = 1$). The shaded gap is the bias $\omega(s) - \omega'(s)\,s$. The smoother model (Gaussian RBF, green) has a more linear modulus and smaller bias than the rougher model (Mat\'ern-3/2, blue).}
\label{fig:modulus}
\end{figure}
\begin{equation}
\label{eq:donoho-liu-bias}
\imbalance_\model(\hgamma) = \omega(s) - \omega'(s)\,s \qwhere \omega(s) = \sup_{m \in \model} \left\{ \frac{1}{n}\sum_{i=1}^n m(X_i) - \frac{1}{2sn} \sum_{i=1}^n m(X_i)^2 \right\}.
\end{equation}
When $\omega$ is approximately linear, the maximal imbalance is approximately zero. Centering $\omega$ at $\gammaipw$ separates the linear growth from the deviation. Define the \emph{offset complexity}
\begin{equation}
\label{eq:offset-complexity}
\xi(s) = \sup_{m \in \model} \left\{ \frac{1}{n}\sum_{i=1}^n \{1 - W_i \gammaipw(X_i)\}\, m(X_i) - \frac{1}{2sn}\sum_{i=1}^n m(X_i)^2 \right\}.
\end{equation}
Because $\E[\{1 - W_i \gammaipw(X_i)\}\, m(X_i)] = 0$ by the population balance property~\eqref{eq_mipw-balance}, the integrand is mean-zero: $\xi$ is a centered empirical process penalized by a quadratic. The maximal imbalance depends only on $\xi$, not on the linear part of $\omega$:
\begin{equation}
\label{eq:bias-centering}
\imbalance_\model(\hgamma) = \omega(s) - \omega'(s)\,s = \xi(s) - \xi'(s)\,s.
\end{equation}
When $\gammaipw$ is in the model, $\xi = 0$. When it is not, bounding the maximal imbalance reduces to bounding~$\xi$.
\begin{figure}[h!]
\centering
\includegraphics[width=0.45\textwidth]{figure/bias-bound.pdf}
\caption{Bias bound $\xi(s) + (\sigma^2/2n)\,B\,(1-s\,B)$, optimized over the competitor $g$ at each~$s$ (Mat\'ern-3/2, $n = 500$, $\sigma = 3$). The dotted line is $n^{-1/2}$. The bound stays below $n^{-1/2}$ for moderate~$s$, locating the operating point in the nearly-linear regime of Figure~\ref{fig:modulus}.}
\label{fig:bias-bound}
\end{figure}

\citet{kong2025asymptotics} and \citet{hirshberg2026bregman} bound $\xi(s)$ by two terms. The first is the local complexity of the model at the fixed-point radius $r$ satisfying~\eqref{eq:least-squares-fixed-point}---the same localization as in Section~\ref{sec:role-of-augmentation}, illustrated in Figure~\ref{fig:width-geometry}, and bounded by $C\,r^2$. The second reflects how well $\gammaipw$ can be approximated in the model: for any $g \in \model$ with $\norm{g}_\model \le B$ and $\norm{g - \gammaipw}_{L_2(P)} \le \varepsilon$, this approximation term is $(\sigma^2/2n)\,B\,(1 - s\,B)$. The bound holds at every~$s$ and for any such competitor~$g$; minimizing over both gives the tightest result. Because the local complexity term grows with~$s$ while the approximation term shrinks, the minimum balances the two---and locates the operating point on the modulus curve. For nonparametric models, the approximation term is negligible for any~$s$: it suffices that \emph{some} function in the model approximates $\gammaipw$ at finite model norm, which is automatic whenever $\gammaipw$ is square-integrable. In this case the binding constraint is just the local complexity $C\,r^2$, the same quantity that governs regression in Section~\ref{sec:role-of-augmentation}. The operating~$s$ falls in the range where $\omega$ is nearly linear in Figure~\ref{fig:modulus}---the shaded gap is small. To match the local complexity rate~$C\,r^2$, the stronger condition $B \le C\,r^2 n/\sigma^2$ is needed; this is the approximation condition in Proposition~\ref{prop:bias-balance}. When both terms are $O(r^2)$, the total is $C\,r^2 = o_p(n^{-1/2})$, and the conclusion is stronger than in Section~\ref{sec:role-of-augmentation}: there, augmentation with a consistent estimator was needed to convert an $O_p(n^{-1/2})$ bound on maximal imbalance into $o_p(n^{-1/2})$. Here, the minimax balancing weights achieve it without augmentation.
