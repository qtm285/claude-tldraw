...
### Screenshot 5: +HUD active — fleet-chat + fleet-agents in layout
![HUD Layout](/Users/skip/work/tlda/.worktrees/w3a-playback/scratch/pbf-5-hud-layout.jpeg)

After clicking `+HUD`, both child shapes are created inside the PlaybackFrame:
- **Left panel** (`FleetChatShape`): shows chat messages from the recording, filtered to `currentMs`
- **Right panel** (`FleetAgentsShape`): agent roster with 5 agents, each showing session ID and task status (`APPENDIX`, `now`)
- `+HUD` button in header creates both shapes at `x=0,y=100` (chat) and `x=508,y=100` (agents) in parent space

### Screenshot 6: Timewarp cut selector
![Cut Selector](/Users/skip/work/tlda/.worktrees/w3a-playback/scratch/pbf-6-timewarp-selector.jpeg)

"The Identity Crisis — Director's Cut" loaded. Header shows:
- **CURATED** badge (purple) — shape is in curated playback mode
- **`Directo ▼`** dropdown — the cut selector, auto-activated to "Director's Cut (~43 min)" timewarp
- Recording has 2 named cuts: "Director's Cut (~43 min)" and "Constant Pace (~47 min)"
- `extractTimewarps()` reads `op:'timewarp'` edits from recording data; cut selector only appears when timewarps exist

### Screenshot 7a: Just before timewarp jump (29:57)
![Before Jump](/Users/skip/work/tlda/.worktrees/w3a-playback/scratch/pbf-7a-before-jump.jpeg)

Seeked to `t=1,797,000ms` (`29:57`). The next timewarp region starts at `t=1,800,000ms` with `speed=500` (>100x). Scrubber sits just before that boundary.

### Screenshot 7b: After jump snap (1:01:17)
![After Jump](/Users/skip/work/tlda/.worktrees/w3a-playback/scratch/pbf-7b-after-jump.jpeg)

After ~2s of play, the RAF tick hit the `1,800,000–3,600,000ms @ speed=500` region. `getEffectiveSpeed()` returned 500 (>100), triggering the jump-snap: `currentMs` was set to `region.end_ms = 3,600,000ms` instantly, then continued at the next region's speed. Final time: `1:01:17` — jumped **31 minutes** of recording time in under a second of wall time.

### Screenshot 8: Curated mode — layout keyframe framework active
![Curated Mode](/Users/skip/work/tlda/.worktrees/w3a-playback/scratch/pbf-8-curated-layout.jpeg)

Director's Cut in `mode: 'curated'`. **CURATED** badge visible in header. `getLayoutKeyframe()` fires on every `currentMs` update — when `type:'layout'` events with `data.shapes` exist in the recording, it calls `editor.store.mergeRemoteChanges()` to reposition child shapes. The Director's Cut has 10 editorial layout events; current recordings use `data.panels` (old dockview format) rather than `data.shapes`, so shape repositioning fires but has no effect — the hook is wired, awaiting recordings with the new format.
