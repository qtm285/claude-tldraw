...
interface PendingWaiter {
  ...
  agent?: AgentRecord
  seatObservation?: AcceptedSeatObservation
  isSeatCurrent?: (seat: AcceptedSeatObservation) => boolean
}

export interface AcceptedSeatObservation {
  agent_id: string
  session_id: string
  daemon_key: string
...
export class SpawnLibrarian {
  ...
  awaitAcceptedSeat(input: { id: string; name: string; spec?: SpawnSpec; isSeatCurrent: (seat: AcceptedSeatObservation) => boolean }): Promise<SpawnResult> {
    return this.awaitReadiness(input, 'accepted-seat', input.isSeatCurrent)
  }

  private awaitReadiness(
    input: { id: string; name: string; spec?: SpawnSpec },
    mode: PendingWaiter['mode'],
    isSeatCurrent?: PendingWaiter['isSeatCurrent'],
  ): Promise<SpawnResult> {
    ...
    return new Promise((resolve) => {
      ...
      this.waiters.set(input.id, { resolve, timer, mode, isSeatCurrent })
    })
  }
  ...
  observeLogin(agent: AgentRecord): void {
    ...
    if (waiter) {
      if (waiter.mode === 'accepted-seat') {
        waiter.agent = agent
        if (waiter.seatObservation && waiter.isSeatCurrent?.(waiter.seatObservation)) {
          this.resolveAcceptedSeat(agent.id, waiter)
        }
        return
      }
  ...
  observeSeat(seat: Partial<AcceptedSeatObservation>): void {
    const agentId = seat.agent_id
    if (!agentId || !seat.session_id || !seat.daemon_key || !seat.terminal_capability || !seat.activated_at) return
    const waiter = this.waiters.get(agentId)
    if (!waiter || waiter.mode !== 'accepted-seat') return
    waiter.seatObservation = seat as AcceptedSeatObservation
    if (!waiter.agent || !waiter.isSeatCurrent?.(waiter.seatObservation)) return
    this.resolveAcceptedSeat(agentId, waiter)
  }

  private resolveAcceptedSeat(agentId: string, waiter: PendingWaiter): void {
