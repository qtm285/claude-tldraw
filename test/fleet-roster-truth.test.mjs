...
test('fleet roster truth separates registry liveness from stale tmux panes', () => {
  const roster = [
    { id: 'fleet:a1', friendly_name: 'alpha', status: 'awake', dead: false, human: false, machine_id: 'mini', tmux_session: 'fleet-alpha', last_seen: '2026-06-26T12:00:00.000Z', metadata: { inboxStatus: 'busy', inboxStatusTag: 'RC', deliveryChannel: 'tmux' } },
  ...
  assert.equal(summary.agents.find(a => a.name === 'alpha').delivery_channel, 'tmux')
  assert.deepEqual(summary.summary.inbox_statuses, [
    { value: 'busy', count: 1 },
    { value: 'dnd', count: 1 },
  ])
  assert.deepEqual(summary.summary.delivery_channels, [
    { value: 'tmux', count: 1 },
  ])
