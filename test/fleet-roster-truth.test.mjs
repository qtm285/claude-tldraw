...
test('fleet roster truth separates registry liveness from stale tmux panes', () => {
  const roster = [
    { id: 'fleet:a1', friendly_name: 'alpha', status: 'awake', dead: false, human: false, machine_id: 'mini', tmux_session: 'fleet-alpha', last_seen: '2026-06-26T12:00:00.000Z', metadata: { inboxMode: 'focus' } },
    { id: 'fleet:a2', friendly_name: 'bravo', status: 'hibernating', dead: false, human: false, machine_id: 'mini', tmux_session: 'fleet-bravo', last_seen: '2026-06-26T12:00:00.000Z', metadata: { inboxMode: 'monitoring' } },
  ...
  assert.equal(summary.agents.find(a => a.name === 'alpha').inbox_mode, 'focus')
  assert.deepEqual(summary.summary.inbox_modes, [
    { value: 'focus', count: 1 },
    { value: 'monitoring', count: 1 },
  ])
