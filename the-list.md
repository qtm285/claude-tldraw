...
**61 things**, from 92 at the start of the night. **The difference is not one subtraction:** 41
...
gone — the arXiv build, a feature an agent invented, and a row built from an example in the README. **The other 61 are things you asked for.**

Your box runs `2e763c010`, read from `/api/build-info` at 09:57 EDT; `main` is `d55fd9e57`, 74 ahead —
...
| | | |
|---|---|---|
| 🔨 | **A squashed merge is invisible to the merged-ness check this repo relies on** | **`AGENTS.md` makes the house rule *check by message*, because `main` is assembled by cherry-pick and ancestry lies. A squash-merge defeats that too** — the branch's subjects never appear on `main`, so the only check the project trusts returns nothing for work that landed. **Measured, not argued:** `sync-ledger`'s subjects all return `NO` against `main`, **while its content is plainly there** — `staleSourceSyncEntries` and `recordSourceSyncRefusal` in `server/lib/source-sync-conflicts.mjs`, `noteRoomIsHolding` in `source-room-daemon.mjs`, `sweepStuckSourceSync` in `unified-server.mjs`. **Subject says never landed; content says landed.** **Three branches are already in that state tonight** — `chat-anchored-scroll` twice and `sync-ledger` — **so an audit of merged-ness would report them as unlanded**, which is exactly the false negative the rule exists to prevent, arriving by a route the rule does not cover. **`chat-anchored-scroll` is partly visible** — one subject does match — **so the check is unreliable rather than uniformly broken, which is worse to audit with.** **The practice changing from here:** a squash commit lists the original subjects in its body, which keeps the check working and costs a paragraph. **Where it has already happened the branch names and head shas are in the bodies, so the trail is recoverable by reading and not by grepping.** *Leaves the list when a subject search against `main` finds the work in every squashed branch.* `app-fix-forward` |
