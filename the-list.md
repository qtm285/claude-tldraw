...
| | | |
|---|---|---|
| 🔨 | **Put a real paper on sync and watch it** | **Ours, being done now.** Nobody has ever done it: the acceptance pass ran on a probe project, not a paper anyone writes in. Tests exist and are green, the editor is proven over the wire against the deployed build, and tonight a real paper with figures went through the push route both directions. **What is left is a paper being written in while somebody watches.** Two earlier versions of this row were wrong — one said nobody was on it, and the one that replaced it said it was waiting on your order, which made you the holdup for something never put to you. `sync-pm` |
