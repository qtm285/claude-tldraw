...
| ○ | **The roster cannot show a dead agent, so its "0 dead" is structural** | **Established from the code, not inferred.** `/api/fleet-table` calls `getAliveAgents()`, which is `WHERE agents.dead = 0` — **dead agents are excluded by construction**, and `totals.dead` is then computed from a set they cannot be in. All 2,154 rows come back `dead: null`. You say there are thousands and the roster says none; **the roster is answering a question it cannot see.** A comment on that very route already records the same shape for never-booted shells — *"the totals previously summed to a number these rows were absent from, which is what made '0 dead, N total' look like proof that nothing had gone missing"* — and it was never extended to the dead. **This is the precondition for the row below:** nobody can reanimate what nobody can see. **Nobody looked** until you said so. Unowned |
