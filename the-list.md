...
**54 things**, from 92 at the start of the night. **The difference is not one subtraction:** 41
...
**8 of the 54 are parked rather than closed — you tabled or deferred them, which is not the same as done.**
...
gone — the arXiv build, a feature an agent invented, and a row built from an example in the README. **The other 54 are things you asked for.**

Your box runs `2e763c010`, read from `/api/build-info` at 08:05 EDT; `main` is `4f6709114`, 32 ahead —
...
| | | |
|---|---|---|
| ? | **Every CLI command should be idempotent and keep going until it is done** | **Your words this morning:** *"we also specified that, like, basically, all of the fucking CLI commands should be, like, idempotent and just, like, keep trying to do the thing they're fucking supposed to do. Until it's fucking done. And, like, if you wanna back out, you fucking control c or control z."* Then: *"I don't know if those fucking ever made it onto a list. I don't know what the fuck happened with that, but, like, it's fucking important."* **The way out is your interrupt, not the command giving up** — that is part of the spec, not a detail. **Marked `?` rather than not-started because nobody has established whether any of it was built**, and saying *nobody looked* would be asserting an absence nobody checked. **No owner** — this row would rather sit honestly unowned than carry a name. |
