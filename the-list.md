...
| 🔨 | **A list component that behaves** — one abstract component with a signature, everything implements it | Your 23:27 design, on `rc/anchored-list`, none of it on `main` or your box. **Measured: 16 files, +1796/−1240** — my earlier 18/+2463/−1198 was wrong. It also deletes two test files, so it removes the machinery and its coverage together. **It does not fix "can't scroll up", and asking settled that rather than assuming it** — the buffer, the trim and the history cursor are all upstream of everything it changes, so it pages into the same hole. **The two are independent and both are needed.** Which also means your symptom may have had two causes the whole time, and no amount of anchor telemetry could ever have shown the buffer one. `list-component` |
