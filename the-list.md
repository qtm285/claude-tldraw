...
invented — the arXiv build — and it is gone. **The other 52 are things you asked for.**

Your box runs `2e763c010`, read from `/api/build-info` at 07:29 EDT; `main` is `46f5fdd97`, 23 ahead —
...
| 🔨 | **Image references in chat carry a giant absolute-path URL** | **Your words:** *"using our ref url system as a url instead of a giant fucking like 'abspath url'"*, with the target form you gave: `![](image#546456)`. **You ranked it low and did not park it:** *"just nice to have because like, it doesnt totally take over your chat."* So it is not urgent and it is not tabled — somebody is on it. *Leaves the list when an image reference in chat is a short ref rather than a path.* `image-ref-url` |
