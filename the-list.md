...
**56 things**, from 92 at the start of the night. **The difference is not one subtraction:** 41
...
**8 of the 55 are parked rather than closed — you tabled or deferred them, which is not the same as done.**

**⚠️ built and finished, but not on your box — 2 rows carry this**, the scrollback work and the
anchored chat list, both merged after the 06:04 deploy. From where you sit it is indistinguishable from not-done. · **✓? built and on your box, and
...
| | | |
|---|---|---|
| ⚠️ | **The anchored chat list mixes screen and layout pixels, so it should misbehave at any zoom but 1** | **A defect in `f34e43f77`, the anchored chat list that merged tonight — on `main`, not on your box.** Three quantities in `src/shapes/FleetChatShape.tsx` meet in the same expressions and they are not in the same coordinate space: `heightByKeyRef` is fed `row.getBoundingClientRect().height` (**:2391, screen space, transform-scaled**), `viewportHeight` is fed `el.clientHeight` (**:2340, layout space, unscaled**), and `geometry.total` sums the scaled heights (**:2288**). **`clampTop` at :2293 subtracts the unscaled term from the scaled sum.** A chat panel renders inside tldraw's camera-transformed layer, **so at any zoom but 1 those are different numbers.** **No normalisation exists** — the file's only two camera reads are at :1149 and :1174, in a telemetry block a thousand lines away. **Status, which is the part that matters: established by reading, never observed running.** **It is still open rather than answered because the falsifier came back blind:** the run got 0px at both zooms, which reads as a clean negative, and the rig then turned out to have zero chat panels, zero rows, an unreachable editor, and `scale` reading 1 before and after the ctrl+wheel — **the camera never moved, so the 0px was the absence of a subject.** **The rig to settle it already exists and costs one keyword:** `AnchoredChatList` at :2255 is `forwardRef` and **not exported**, with generic props, and there is a harness with 500 unequal rows and a 0.5×/1×/2× wrapper — **export it and the falsifier runs without the app.** *Leaves the list when a chat panel is scrolled at zoom 2 and the content tracks the pointer.* `chat-jerks` |
| 🔨 | **Ghost text is painted at the active tone, so it reads as real text** | **Your words:** *"ghost text in search/agents panel etc is so fucking prominent it's easily confused w non-ghost."* **Both sites confirmed in `src/shapes/fleet-chat.css`:** the agents-panel spawn ghost (line 2699) is `color: var(--text-dim)` with `opacity: var(--fleet-agents-content-active)` — **the *active* content tone, on a thing meant to recede**; the filter chip ghost (line 355) is `opacity: var(--ui-tone-rest)`, 0.55, with a dashed border. **Same class as the go-there arrows: a recede-thing given operate-weight.** **And the `faint` preference barely reaches either, which is now established rather than an open question:** `.fleet-faint` redefines only the `--fleet-dim-alpha-NN` family and `--text-dim` (lines 555–581). So the spawn ghost's **colour** follows `faint` and its **opacity** does not, and the filter chip ghost is untouched by it entirely — `--ui-tone-rest` is not in that family. *Leaves the list when ghost text reads as ghost at a glance, and follows `faint` where the rest of the shape does.* `ghost-tone` |
...
| 🔨 | **The layout button after a vertical split crashes the page** | **Fixed and on `main`: `0f8b13c13`**, *"the doc viewer's layout button no longer remounts it to death"*, one file, `FleetDocViewShape.tsx`. **On your box now — the deploy at 06:04 serves `2e763c010`, which contains it.** **Landed and unverified.** A control build read identical to the treatment, so that run measured nothing. **What the next attempt needs first:** `FleetDocViewShape.tsx:711` mounts `CanvasClipPanel` only when `bounds && docviewSurface && mainEditor && (targetShapeId \|\| svgReady)`, so **a default-layout docview has no target and no bounds and the nested viewport never mounts** — the ⊞ click engaged on a panel with nothing inside it. **`nestedViewport: false` is the discriminator: assert it is `true` before measuring, rather than discovering afterwards that it was not.** The measurement that stands: 51 mounts against React's 50-deep limit on `eiv-paper`, with a chat-panel control that did not crash. **Reproduced, with a cause and a control.** On `eiv-paper`: **51 remounts against React's limit of 50.** The control — the same sequence on a chat panel — does not crash. **The row said nobody looked and that is no longer true.** Not fixed. *Leaves the list when the sequence stops remounting past the limit and you can split without it going white.* `canvas-bugs` |
...
| 🔨 | **Place-stack forward/back over documents** | **Your feature, your words:** *"we maintain a place stack… a browser has that. Where a place is a document."* **The branch exists — `place-stack-nav`, one commit, 313 behind `main`** — so *never landed* was true and *no branch* is not. **It conflicts in `HtmlPageShape.tsx`**, found by asking which files `main` has rewritten since the branch forked rather than by attempting the merge. `sync-pm` is landing it. *Leaves the list when forward and back move you between documents.* `sync-pm` |
| 🔨 | **Pen/handwriting correction** | **The branch exists and does not merge** — `rc/pen-correction`, 411 behind `main`, colliding in `FleetChatShape.tsx`, **which `main` has rewritten in 46 commits since it forked** and which two agents rewrote again tonight. **The finding worth keeping, because it dies with the branch otherwise:** while the pen is active chat sets `pointer-events: none`, so **`elementsFromPoint` and `caretRangeFromPoint` see straight past chat to the canvas** — which is why that branch's own first commit could never work, and its second commit says so. **Browser-verified. Whoever re-lands the idea should not rediscover it.** *Leaves the list when a pen correction lands on the text it was drawn over.* `sync-pm` |
...
| 🔨 | **Token permissions — the switch for whether the app needs a token** | **The branch exists and merges clean — `fix/token-gating`, 284 behind `main` — and it was still not ready, for a reason no merge check could see.** `config/deployments/pic/server.yaml` **did not exist when the branch forked**; it sets `authDisabled: true` at line 23, and the branch **deletes the code that reads that key.** **Not a security regression** — the course box is open before and after — **but a dead config key on a live deployment**, which is the settings-control failure this repo already has a document about, this time in a deployment file. **The fix is to state `tokenGating: false` explicitly and drop `authDisabled`, behaviour unchanged.** `sync-pm` is landing it. *Leaves the list when the switch works and no deployment file names a key nothing reads.* `sync-pm` |
