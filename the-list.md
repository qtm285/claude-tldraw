...
Your box runs `2e763c010`, read from `/api/build-info` at 06:49 EDT; `main` is `4c1501876`, 18 ahead — all
...
| 🔨 | **Sometimes you can't scroll up, and it's worse than it used to be** | **A cause, and it matches the symptom:** `requestEarlierChatHistory` now reads the scrollback boundary **from the buffer at the moment you reach the top**, rather than the subscription remembering where it was — *"sometimes I can't scroll up"* is what a cursor held somewhere other than the hole looks like. On branch `chat-scroll-history-hole`, head `2a80c7817`, nine files, merging when the typecheck clears. **Its two halves are not equally established and are not averaged here.** **Run, not read:** the block-scoped history query against a real store, the `subscribe-filter` wire end to end, a counterfactual showing the block path is what executes, and a critic pass that found and fixed two defects. **Landed but unwatched:** `3abc405bb`, the client-side scrollback boundary in `FleetChatShape.tsx` — typecheck only, **no panel has scrolled with it.** **Two earlier mechanisms on this row are dead:** the scrollbar-drag one, because you are on Safari and there is no scrollbar, reverted off `main`; and the buffer-trim one, because your sessions never reached the 500-event cap it depended on. *Leaves the list when you scroll up and the history is there.* `chat-scroll` |
