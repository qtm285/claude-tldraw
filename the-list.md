...
| ○ | **An endpoint that paginates must say so — `roster` and the history endpoint both fail to** | **This row cost you something tonight, hours after it was written.** I asked the fleet table how many dead agents there are, read the 500 rows it returned, and told you zero. There are 2,154 rows across five pages and it never said there was a second one. **The row was already on your list saying nobody had looked, and then it happened to me while I was using the list.** `thread` announces its pages; `roster` and the history endpoint do not. **Nobody has looked at the code** |
