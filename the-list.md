...
Your box runs `2e763c010`, read from `/api/build-info` at 08:29 EDT; `main` is `f497a7755`, 39 ahead —
...
| 🔨 | Place-stack forward/back over documents | Built on a branch, never landed. **Owned now: merge-readiness, not merging** — the owner reports whether it lands cleanly and `app-fix-forward` gates the merge. **If the branch turns out not to exist, that goes on this row as the finding**, not as a closure. `sync-pm` |
| 🔨 | Pen/handwriting correction | Built on a branch, never landed. **Owned now: merge-readiness, not merging** — the owner reports whether it lands cleanly and `app-fix-forward` gates the merge. **If the branch turns out not to exist, that goes on this row as the finding**, not as a closure. `sync-pm` |
