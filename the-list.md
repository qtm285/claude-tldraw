...
Your box runs `2e763c010`, read from `/api/build-info` at 09:52 EDT; `main` is `06519a184`, 70 ahead —
...
| 🔨 | **The layout button after a vertical split crashes the page** | **The fix was reverted on `main` (`796cbb755`) and the revert has not deployed — so your box is running the reverted code right now.** `0f8b13c13` is in `2e763c010`; the revert is not. **Why it came off:** starting `svgReady` true removed the state flip that made each iteration a nested update, and React's depth limit only fires for nested updates inside one pass — **so if anything else still drives the remount, the loop runs as fresh mounts with no bound at all.** The reverted commit says itself that what remounts the shape was never established: **it removed the bound, not the loop.** **The evidence is an asymmetry, not a count:** the control crashes and **stays responsive** — a selection was cleared through it — while the fix arm never answers, and **zero React #185 in the fix arm is the absence that says the bound is gone.** **Read honestly as a read:** no payload from the fix arm so there is no mount count, load rose 7.8 → 22.8 during the runs, and `TypeError: Failed to fetch` was present so a fetch-retry storm is not excluded. **`canvas-bugs` cleared storage between arms, which is what removed the sticky-selection confound that had killed two earlier attempts.** **The branch is kept.** **This is a holding position, not a fix.** *Leaves the list when a fix-arm run with a healthy proxy shows the renderer idle, or when the revert reaches your box.* `canvas-bugs` |
