...
test('Deepgram stale tab cannot keep typing through the Enter boundary', async () => {
  installBrowserGlobals()
  const voice = await import('../src/voice.mjs')
  const { __test, setVoiceTarget, completeMessageSend } = voice
  ...
  __test.forceDeepgram()
  __test.setRecording(true)
  __test.writeVoiceMicOwnerForTest({ id: __test.micOwnerId(), at: Date.now(), reason: 'test-current' })
  ...
  const epoch2 = __test.state().speechEpoch
  __test.writeVoiceMicOwnerForTest({ id: 'newer-tab', at: Date.now(), reason: 'test-stale' })
  for (let i = 0; i < 4; i++) {
    __test.deepgramMessage({ type: 'transcript', epoch: epoch2, text: 'alpha beta', is_final: true })
  }

  assert.equal(textarea.value, '')
  assert.equal(__test.state().recording, false)
  assert.deepEqual(submitted, [{ text: 'alpha beta', submittedText: 'alpha beta' }])
  assert.equal(__test.boundaryTelemetry().messageSends, 1)
})
