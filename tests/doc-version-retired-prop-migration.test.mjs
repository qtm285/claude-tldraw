...
test('retired sourceVersion is removed without changing current revision props', () => {
  const stored = {
    w: 1,
    h: 1,
    commitHash: 'abc123',
    timestamp: 42,
    sourceVersion: 1786752487,
    sourceRevision: 'revision-7',
    acceptSeq: 7,
  }

  assert.deepEqual(stripRetiredDocVersionSourceVersion(stored), {
    w: 1,
    h: 1,
    commitHash: 'abc123',
    timestamp: 42,
  ...
  assert.equal(stored.sourceVersion, 1786752487, 'migration must not mutate its input')
})
...
test('client and sync-room schemas apply the same doc-version migration', () => {
  ...
  assert.equal(DOC_VERSION_SOURCE_VERSION_MIGRATION_ID, 'com.tldraw.shape.doc-version/2')
  ...
  for (const source of [client, server]) {
    ...
    assert.match(source, /DOC_VERSION_SOURCE_VERSION_MIGRATION_ID/)
    assert.match(source, /stripRetiredDocVersionSourceVersion/)
  }
