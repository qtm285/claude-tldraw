...
function skillNameFromReadPath(p) {
  const m = String(p || '').match(/[/\\]skills[/\\]([^/\\]+)[/\\]SKILL\.md$/)
  return m?.[1] || ''
}
...
if (toolName === 'Read') {
  if (!skillNameFromReadPath(filePath)) process.exit(0)
  await httpGet(url, { timeoutMs: 10000 })
  process.exit(0)
}
if (toolName === 'Skill') { await httpGet(url, { timeoutMs: 10000 }); process.exit(0) }
