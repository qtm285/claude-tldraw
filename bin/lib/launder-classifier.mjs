const MATH_SYMBOL_RE = String.raw`(?:\\[A-Za-z]+(?:\s*\{[^}]{1,20}\})?(?:\s+(?:[A-Za-z]|[\u0391-\u03ff])[\p{L}\u0300-\u036f]*(?:[_^][A-Za-z0-9{}\\]+)?)?(?:\s*\([^)]{0,20}\))?|[A-Z][\p{L}\u0300-\u036f]*(?:[_^][A-Za-z0-9{}\\]+)?(?:\s*\([^)]{0,20}\))?|[a-z](?:[_^][A-Za-z0-9{}\\]+)?(?:\s*\([^)]{0,20}\))?|[\u0391-\u03ff][\p{L}\u0300-\u036f]*(?:[_^][A-Za-z0-9{}\\]+)?(?:\s*\([^)]{0,20}\))?)`
const CONTENT_WORD_RE = String.raw`(?!(?:the|a|an|as|for|to|be|is|are|and|or|of|in|on|with)\b)[a-z]+(?:-[a-z]+)*`
const TERM_OBJECT_RE = String.raw`${CONTENT_WORD_RE}(?:\s+${CONTENT_WORD_RE}){1,5}`
