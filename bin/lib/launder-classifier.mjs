...
const MATH_BIND_RE = new RegExp(String.raw`\b(?:(?:[Ll]et|[Dd]efine)\s+(${MATH_SYMBOL_RE})\s+(?:be|=|:=|\bas\b)|[Dd]enote\s+by\s+(${MATH_SYMBOL_RE})(?=\s|[.,;:)]|$)|[Dd]enote\s+(${MATH_SYMBOL_RE})\s+(?:as|to be)|(${MATH_SYMBOL_RE})\s*:=)`, 'u')
