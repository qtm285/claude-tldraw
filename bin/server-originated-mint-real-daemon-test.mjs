...
const DB = `/tmp/server-originated-real-mint-${process.pid}.db`
const CONFIG_DIR = `/tmp/server-originated-real-mint-config-${process.pid}`
const PROJECTS_DIR = `${CONFIG_DIR}/projects`
const ENV_NAME = 'test'
const MACHINE_ID = `real-mint-box-${process.pid}`
const REQUESTER_ID = 'fleet:real-mint-requester'
const REQUESTER_NAME = 'real-mint-requester'
const MINT_NAME = `real-mint-gate-${process.pid}`
const PHRASE = `real-mint-proof-${process.pid}`
...
`)
  ...
  daemon = spawnProcess(process.execPath, ['bin/fleet-daemon.mjs'], {
    ...
    env: {
      ...
      CODEX_SESSIONS_DIR: `${CONFIG_DIR}/codex-sessions`,
    },
    stdio: ['ignore', 'pipe', 'pipe'],
  })
  ...
  const shell = await stateAgent(a => a.id === agentId && a.friendly_name === MINT_NAME, 15_000)
