...
import { classifyUserBlame } from './lib/user-blame-classifier.mjs'
import { userBlameExamples } from '../test/fixtures/user-blame-examples.mjs'
...
function usage(code) {
  console.log(`Usage: node bin/user-blame-eval.mjs [--input labeled.jsonl] [options]
...
function loadRows(input) {
  if (!input) {
    return userBlameExamples.map(example => ({
      eventId: example.id,
      text: example.text,
...
function evaluate(rows) {
  ...
  for (const row of rows) {
    const actual = classifyUserBlame({ text: row.text, context: row.context || {} })
...
function main() {
  ...
  console.log(JSON.stringify({
    input: args.input || 'test/fixtures/user-blame-examples.mjs',
    seed: args.seed,
    split: {
      train: train.length,
      test: test.length,
