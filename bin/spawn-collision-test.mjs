  ...
  return new Promise((resolve) => {
    ...
    dws.on('open', () => {
      dws.send(JSON.stringify({
        type: 'daemon-hello', machine_id: 'testbox', boot_id: Date.now(),
...
async function run() {
  ...
  ws.send(JSON.stringify({ type: 'register', agent_id: 'fleet:tester1', name: 'collidertest', machine_id: 'testbox' }))
  await sleep(800)
  ...
  ws.send(JSON.stringify({ type: 'spawn', name: 'totallynewname', model: 'sonnet' }))
  await sleep(1200)
