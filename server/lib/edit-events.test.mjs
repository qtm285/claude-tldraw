...
import { __test as sourceEditTest, acceptSourceTransaction, activeSourceEditors, recordSourceEditActivity } from './source-edit-activity.mjs'
...
test('local checkout Edit activity attributes the watched source transaction to the editing agent', async () => {
  await withProject(async name => {
    sourceEditTest.reset()
    await updateProject(name, { sourceDir: '/work/bregman' })
    const before = await bootstrap(name, 'alpha\n')
    const active = {
      agent_id: 'fleet:b4-live-writer', tool: 'Edit', status: 'started', correlationId: 'edit-b4',
      project: name, sourceFile: 'main.tex',
    }
    recordSourceEditActivity(active)
    recordSourceEditActivity({ ...active, status: 'completed' })
    assert.deepEqual(activeSourceEditors(name, 'main.tex'), ['fleet:b4-live-writer'])
    ...
    assert.equal(acceptSourceTransaction(name, ['main.tex'], payload.events[0].actor_id), true)
    assert.deepEqual(activeSourceEditors(name, 'main.tex'), [])
  })
})
