...
export function acceptSourceTransaction(project, files, actorId) {
  if (!actorId) return false
  const accepted = new Set(files || [])
  let cleared = false
  for (const [key, edit] of activeEdits) {
    if (edit.project === project && edit.agentId === actorId && accepted.has(edit.file)) {
      activeEdits.delete(key)
      cleared = true
    }
  }
