...
export function createShadowMirrorRpcHandler({ readProject, sendRpc, daemonAddressFor = daemonAddress }) {
  return async function mirrorShadowViaDaemon({ name, hash, bundleBase64, sourceScope }) {
    ...
    const result = await sendRpc(daemonAddressFor(machineId, envName), 'mirror-shadow-ref', {
      project: name,
      hash,
      bundleBase64,
      sourceScope,
